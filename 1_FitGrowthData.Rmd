...
We then assign names to the columns of `datain`.  ( Remember the command `c("R","g")` create a vector with two elements : "R" and "g").
...
Now we define a vector of parameter `parms` with guess values and will use it to plot a curve depicting how the growth rate vary with the resource availability. 
...
$L_1(\Theta_j) = \sum |S_{i,j}|/n $ and $L_2(\Theta_j) = \sqrt{\sum S_{i,j}^2/n }$
...
Here the transfer function returns an object `ModCost`.
``` {r}
